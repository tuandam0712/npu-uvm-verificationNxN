...
module controller_formal;
    ...
    always @(posedge clk) begin
        C_CTRL_CLEAR:
            cover(formal_state == CLEAR_STATE);

        C_CTRL_COMPUTE:
            cover(formal_state == COMPUTE_STATE);

        C_CTRL_DRAIN:
            cover(formal_state == WAIT_DRAIN_STATE);

        C_CTRL_DONE:
            cover(formal_state == DONE_STATE);

        C_CTRL_START_ACCEPT:
            cover(
                past_valid &&
                $past(formal_state) == IDLE_STATE &&
                $past(start) &&
                formal_state == CLEAR_STATE
            );

        C_CTRL_START_BUSY_COMPUTE:
            cover(
                formal_state == COMPUTE_STATE &&
                start
            );

        C_CTRL_START_BUSY_DRAIN:
            cover(
                formal_state == WAIT_DRAIN_STATE &&
                start
            );

        C_CTRL_START_BUSY_DONE:
            cover(
                formal_state == DONE_STATE &&
                start
            );
