...
| Root Cause | The earlier UVM environment expected every started transaction to produce done and an output. Reset aborts the operation, so unconditional waits and stale expected data broke recovery. |
| Current Status | Resolved for directed COMPUTE and WAIT_DRAIN reset at N=8/width=8 on 2026-09-11; each run has 141 PASS, 1 abort, no mismatch. |
| Fix | Driver/monitor task cancellation; scoreboard cancels pending comparison before FIFO flush; exact scenario-based abort/completion checks; reset-state checks and post-reset recovery. |
| Remaining Work | Repeated/random reset timing, near-done races, APB reset recovery, and parameter sweeps. |
...
| Historical Result | The initial error-response milestone passed 1249/1249 transfers. After adding two back-to-back writes, the 2026-09-10 log reports 1251/1251, 384/384 C checks, nine expected errors, one back-to-back cover hit, and zero UVM errors/fatals. |
...
## 19. Test waited for done after the driver had already consumed it

During reset-aware driver integration, the zero test matched once then timed
out. Moving item_done after completion meant seq.start returned after the done
pulse; run_one_sequence then waited for another pulse. The fix wraps seq.start
in a bounded timeout instead. Baseline subsequently passed 142/142.

## 20. Reproducible reset evidence and stale runner paths

The legacy compile script referenced missing sva/pe_sva.sv and pe_bind.sv.
Compiling UVM sources directly also required the UVM DPI DLL at simulation
load. scripts/run_npu_reset.do compiles current embedded SVA, loads the DLL,
uses fixed seed 1, and checks baseline/COMPUTE/DRAIN summaries independently.
scripts/run_uvm.do delegates to baseline. The older scripts/compile.do and
scripts/run_cov.do are legacy flows, not validated reset entrypoints.

The initial reset run produced 141 matches but failed the old 142-only check.
Scenario-based expected aborts now distinguish a legal cancellation from
missing/extra operations. See reports/NPU_RESET_REPORT.md for the final scope.
