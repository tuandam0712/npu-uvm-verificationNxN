...
The Systolic Array directed regression is recorded as 22 checks passed and 0
failed at `N=8`, `width=8`, and `ACC_WIDTH=19`. Its saved SVA report records
1,348 assertion passes, 0 assertion failures, and 100% directive coverage. All
six quick formal tasks at `N=2`, `width=4` and all six exact formal tasks at
`N=8`, `width=8` have `PASS` artifacts. These formal results prove the local
reset, clear, operand-routing, valid-routing, MAC-recurrence, and hold
properties; they do not constitute a single end-to-end proof of matrix
multiplication.
...
| Test ID           | Requirement                                                    | Stimulus                                                                                                                                            | Expected result                                                                                                               | Current implementation |
| ----------------- | -------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------- | ---------------------- |
| `SARR_RST_TC_001` | `SARR_RST_001`, `SARR_RST_002`, `SARR_RST_003`, `SARR_RST_004` | First create nonzero operand-pipeline, local-valid, accumulator, and output state. Assert `rst_n=0` between rising edges while the array is active. | All A/B pipeline state, local-valid state, PE accumulators, and outputs become zero without waiting for the next rising edge. | PASS in the saved directed/SVA run; exact formal task `reset` is `PASS` for the clock-sampled reset properties |
| `SARR_RST_TC_002` | `SARR_RST_005`                                                 | Hold `rst_n=0` for multiple cycles while changing `clear`, `valid_in`, `a_in`, and `b_in`.                                                          | All operand pipelines, local-valid state, accumulators, and outputs remain zero while reset is asserted.                      | PASS in the saved directed/SVA run; exact formal task `reset` is `PASS` |
| `SARR_RST_TC_003` | `SARR_RST_006`                                                 | Assert `rst_n=0` while `clear=1`, `valid_in=1`, nonzero operands are present, and one or more PEs contain partial accumulated results.              | Reset dominates clear, propagation, and MAC behavior. The complete array remains in the reset state.                          | PASS in the saved directed/SVA run; exact formal task `reset` is `PASS` |
| `SARR_RST_TC_004` | `SARR_RST_002` to `SARR_RST_006`                               | Assert reset at several operation points: before valid entry, during early propagation, during middle-array MAC activity, and during drain.         | Reset clears all in-flight state and aborts the current operation regardless of the wavefront position.                       | Partial directed-point coverage; active-state reset passes and exact formal task `reset` is `PASS` for the implemented sampled properties |
...
| Test ID           | Requirement                                                    | Stimulus                                                                                                                | Expected result                                                                                                  | Current implementation |
| ----------------- | -------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- | ---------------------- |
| `SARR_CLR_TC_001` | `SARR_CLR_001`, `SARR_CLR_002`, `SARR_CLR_003`, `SARR_CLR_004` | Create nonzero pipeline, valid, accumulator, and output state. Assert `clear=1` before one rising edge while `rst_n=1`. | All A/B pipeline state, local-valid state, PE accumulators, and outputs become zero on the affected rising edge. | PASS in the saved directed/SVA run; exact formal task `clear` is `PASS` |
| `SARR_CLR_TC_002` | `SARR_CLR_005`                                                 | Hold `clear=1` across multiple rising edges while varying `valid_in`, `a_in`, and `b_in`.                               | All cleared state remains zero after every rising edge for which `clear=1`.                                      | PASS in the saved directed/SVA run; exact formal task `clear` is `PASS` |
| `SARR_CLR_TC_003` | `SARR_CLR_006`                                                 | Assert `clear=1` while `valid_in=1` and active MAC or operand propagation is present.                                   | Clear prevents propagation and MAC updates on the affected edge and has priority over normal array behavior.     | PASS in the saved directed/SVA run; exact formal task `clear` is `PASS` |
| `SARR_CLR_TC_004` | `SARR_CLR_001` to `SARR_CLR_006`                               | Assert clear during early propagation, middle-array MAC activity, and drain.                                            | All in-flight state and partial results are synchronously removed at the next rising edge.                       | Partial directed-point coverage; active clear passes and exact formal task `clear` is `PASS` |
...
| Test ID          | Requirement                | Stimulus                                                                                                   | Expected result                                                                                         | Current implementation |
| ---------------- | -------------------------- | ---------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------- | ---------------------- |
| `SARR_A_TC_001`  | `SARR_A_001`, `SARR_A_002` | Drive unique tagged A values on every row while keeping B and valid inactive. Observe entry into column 0. | `a_in[i]` reaches `PE[i][0]` after exactly `i+1` rising edges.                                          | Exact formal task `operand` is `PASS`; dedicated tagged directed test remains planned |
| `SARR_A_TC_002`  | `SARR_A_003`               | Drive a unique A tag into a selected row and observe consecutive PE columns.                               | The A value propagates exactly one column to the right per rising edge.                                 | Exact formal task `operand` is `PASS`; dedicated tagged directed test remains planned |
| `SARR_A_TC_003`  | `SARR_A_004`               | Drive changing A values while local valid is low.                                                          | A skew and propagation continue independently of local valid until reset or clear flushes the pipeline. | Exact formal task `operand` is `PASS`; dedicated directed test remains planned |
| `SARR_B_TC_001`  | `SARR_B_001`, `SARR_B_002` | Drive unique tagged B values on every column while keeping A and valid inactive. Observe entry into row 0. | `b_in[j]` reaches `PE[0][j]` after exactly `j+1` rising edges.                                          | Exact formal task `operand` is `PASS`; dedicated tagged directed test remains planned |
| `SARR_B_TC_002`  | `SARR_B_003`               | Drive a unique B tag into a selected column and observe consecutive PE rows.                               | The B value propagates exactly one row downward per rising edge.                                        | Exact formal task `operand` is `PASS`; dedicated tagged directed test remains planned |
| `SARR_B_TC_003`  | `SARR_B_004`               | Drive changing B values while local valid is low.                                                          | B skew and propagation continue independently of local valid until reset or clear flushes the pipeline. | Exact formal task `operand` is `PASS`; dedicated directed test remains planned |
| `SARR_AB_TC_001` | `SARR_A_*`, `SARR_B_*`     | Simultaneously drive distinct tagged values across all A rows and B columns.                               | No A value crosses into another row and no B value crosses into another column.                         | Exact formal task `operand` is `PASS`; dedicated simultaneous-tag directed test remains planned |
...
| Test ID           | Requirement                    | Stimulus                                                                                                    | Expected result                                                                                                                                  | Current implementation |
| ----------------- | ------------------------------ | ----------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------- |
| `SARR_VAL_TC_001` | `SARR_VAL_001`, `SARR_VAL_004` | Pulse `valid_in=1` for one cycle after clear and observe representative PE positions.                       | The valid pulse reaches `PE[i][j]` after exactly `i+j+1` rising edges.                                                                           | Exact formal task `valid` is `PASS`; dedicated representative-position directed test remains planned |
| `SARR_VAL_TC_002` | `SARR_VAL_002`, `SARR_VAL_003` | Drive a one-cycle valid pulse and observe propagation through column 0 and then across each row.            | Local valid propagates one hop per rising edge along the specified route.                                                                        | Exact formal task `valid` is `PASS`; dedicated directed observation remains planned |
...
| `SARR_VAL_TC_004` | `SARR_VAL_006`                 | Assert reset and separately assert clear while valid state is present inside the array.                     | Reset asynchronously clears all local-valid state. Clear synchronously clears all local-valid state and blocks propagation on the affected edge. | Saved directed/SVA run passes; exact formal tasks `reset` and `clear` are `PASS` |
...
| Test ID             | Requirement                                          | Stimulus                                                                                                                              | Expected result                                                                                                                        | Current implementation |
| ------------------- | ---------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- | ---------------------- |
| `SARR_ALIGN_TC_001` | `SARR_ALIGN_001`, `SARR_ALIGN_002`, `SARR_ALIGN_003` | Drive uniquely tagged A and B samples on every valid input cycle. Observe local A, local B, and local valid at selected PE positions. | Whenever local valid is high at `PE[i][j]`, local A and B correspond to the same original input sample cycle delayed by `i+j+1` edges. | Partial: exact `operand` and `valid` proofs support local routing, and the saved generated SVA passes; dedicated tagged alignment remains planned |
| `SARR_ALIGN_TC_002` | `SARR_ALIGN_001` to `SARR_ALIGN_003`                 | Use distinct sample indices for all `k` values in one legal operation.                                                                | Each PE receives the operand pairs in the expected `k=0` through `N-1` order without duplication, omission, or reordering.             | Partial: local routing formal tasks pass; dedicated ordered-sample checking remains planned |
| `SARR_ALIGN_TC_003` | `SARR_ALIGN_004`                                     | Propagate nonzero operands through selected PEs while local valid is low.                                                             | PE accumulators do not update even though operand values continue moving through the array.                                            | Exact formal task `mac_hold` is `PASS`; dedicated directed observation remains planned |
| `SARR_ALIGN_TC_004` | `SARR_ALIGN_*`                                       | Use row-unique A tags and column-unique B tags across the entire array.                                                               | No PE combines an A sample from the wrong row, a B sample from the wrong column, or operands from different input cycles.              | Partial: exact local operand/valid routing tasks pass; dedicated combined-tag checking remains planned |
...
| Test ID           | Requirement                    | Stimulus                                                                                                        | Expected result                                                                                       | Current implementation |
| ----------------- | ------------------------------ | --------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------- | ---------------------- |
| `SARR_MAC_TC_001` | `SARR_MAC_001`                 | Clear the array, then apply one valid cycle with simple nonzero operands.                                       | Every PE whose local valid is asserted performs exactly one signed MAC update on that edge.           | Exact formal task `mac_hold` proves the local MAC recurrence; dedicated directed test remains planned |
...
| `SARR_MAC_TC_003` | `SARR_MAC_003`                 | Use a small deterministic matrix with known products and observe selected PE accumulators after each valid MAC. | After the `m`-th MAC, the accumulator equals the fixed-width sum of the first `m` valid products.     | Exact formal task `mac_hold` proves each local recurrence step; dedicated partial-sum directed test remains planned |
| `SARR_MAC_TC_004` | `SARR_MAC_004`                 | Multiply two simple known matrices, such as identity, zero, diagonal, or small signed matrices.                 | Every `c[i][j]` matches the independent reference-model result after completion.                      | PASS for identity and deterministic signed matrices in the saved directed run; no end-to-end formal property is claimed |
...
| `SARR_MAC_TC_008` | `SARR_MAC_005`                 | Create a nonzero result, then keep local valid inactive while changing operands for multiple cycles.            | Every affected accumulator and output retains its previous value.                                     | PASS in the saved directed/SVA run; exact formal task `mac_hold` is `PASS` |
...
| `SARR_TIME_TC_003` | `SARR_TIME_003` | Deassert `valid_in` after the final input sample while operands and valid state remain in flight. | In-flight operands and valid continue propagating until all pending MAC operations complete.       | Partial: the in-flight formal cover is reached in quick and exact profiles; dedicated completion proof/check remains planned |
...
| `SARR_OUT_TC_001`  | `SARR_OUT_001`  | Observe `c[i][j]` during reset, clear, MAC, and hold behavior.                                    | Each `c[i][j]` continuously reflects the corresponding PE accumulator.                             | PASS for reset, clear, MAC, and hold behavior in saved SVA/formal evidence; direct accumulator aliasing is structural |
| `SARR_OUT_TC_002`  | `SARR_OUT_002`  | Create nonzero outputs, then drive cycles with local valid low and reset/clear inactive.          | Outputs remain stable.                                                                             | PASS in the saved directed/SVA run; exact formal task `mac_hold` is `PASS` |
| `SARR_OUT_TC_003`  | `SARR_OUT_003`  | Complete an operation and continue clocking without reset, clear, or new valid MAC activity.      | The full output matrix remains stable after completion.                                            | PASS in the saved directed/SVA run; exact formal task `mac_hold` is `PASS` |
...
| Test ID          | Requirement   | Stimulus                                                                                          | Expected result                                                                                          | Current implementation |
| ---------------- | ------------- | ------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------- | ---------------------- |
| `SARR_OP_TC_001` | `SARR_OP_001` | Complete one operation, assert clear for at least one rising edge, then start a second operation. | The second operation begins with zero operand-pipeline, local-valid, and accumulator state.              | PASS in the saved directed/SVA run; exact formal task `clear` is `PASS` |
| `SARR_OP_TC_002` | `SARR_OP_002` | Run two distinct matrices back to back using the specified clear protocol.                        | No operand, valid, partial sum, or output from the first operation contributes to the second result.     | PASS for identity followed by a signed matrix in the saved directed run; formal `clear` provides local flush evidence |
| `SARR_OP_TC_003` | `SARR_OP_003` | After clear, observe the first valid MAC at selected PE positions.                                | The first valid MAC begins from an accumulator value of zero.                                            | Partial: exact `clear` and local MAC-recurrence proofs pass; a dedicated combined first-MAC check remains planned |
...
| `SARR_PARAM_TC_004` | `N=8`, `width=8`                                 | All applicable directed and constrained-random checks pass for the primary configuration.                                                   | Partial: saved directed/SVA run and all six exact formal tasks pass; constrained-random testing remains planned |
...
The current SARR evidence closes the directed/assertion/local-formal milestone:
22 directed checks pass with zero failures, all saved simulation assertions
pass, and all six quick plus all six exact 8x8 formal tasks pass. Full SARR
scope remains open for the planned constrained-random, functional-coverage,
parameter-sweep, boundary/wraparound, exact-count, and completion-timing cases.
...
| 0.8 | 2026-08-14 | Recorded the saved SARR directed/SVA results and the passing quick and exact 8x8 formal evidence; retained explicit gaps outside the local-formal milestone. |
